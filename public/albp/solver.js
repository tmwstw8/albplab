...
function assignmentFeasible(problem,stations){
  const route=new Map(),known=new Set(problem.tasks.map(t=>t.id)),K=stations.length;
...
export function evaluateSolution(problem,stations,sequence){
  const L=sequence.length,K=stations.length,task=new Map(problem.tasks.map(t=>[t.id,t])),at=n=>sequence[((n%L)+L)%L],cycleLoads=stations.map(()=>Array(L).fill(0));
  const backStationCount=stations.filter(station=>station.backward.length>0).length;
  for(let j=0;j<K;j++)for(let r=0;r<L;r++){const fm=at(r-j+backStationCount-2),bm=at(r+j);cycleLoads[j][r]=stations[j].forward.reduce((s,id)=>s+task.get(id).times[fm],0)+stations[j].backward.reduce((s,id)=>s+task.get(id).times[bm],0);}
  const dt=problem.models.reduce((s,m)=>s+m.demand,0),weightedTotal=problem.tasks.reduce((sum,t)=>sum+problem.models.reduce((s,m)=>s+m.demand*t.times[m.id],0)/dt,0),cmin=weightedTotal/K;
  let overload=0,adw=0,maxLoad=0;cycleLoads.forEach(row=>row.forEach(w=>{overload+=Math.max(0,w-problem.cycleTime);adw+=Math.abs(w-cmin);maxLoad=Math.max(maxLoad,w);}));
  const precedenceFeasible=assignmentFeasible(problem,stations);return{stations,sequence,cycleLoads,stationCount:K,backStationCount,cmin,overload,adw,maxLoad,precedenceFeasible,feasible:overload<1e-9&&precedenceFeasible,efficiency:weightedTotal/(K*problem.cycleTime)};
}

function averageTaskLoad(problem,id){const task=problem.tasks.find(t=>t.id===id),total=problem.models.reduce((sum,m)=>sum+m.demand,0);return problem.models.reduce((sum,m)=>sum+m.demand*task.times[m.id],0)/total;}
...
function collector(algorithm,run,limit=2000){
  ...
  return{add(solution,iteration){const key=solutionKey(solution),old=map.get(key);if(!old||better(solution,old.solution))map.set(key,{algorithm,run,iteration,solution});if(map.size>limit+Math.max(25,Math.ceil(limit*.1)))trim();},values(){trim();return[...map.values()];},dropped(){return dropped;}};
}
...
function stationLoad(problem,ids){return ids.reduce((sum,id)=>sum+averageTaskLoad(problem,id),0);}
function topologicalOrder(problem){const done=new Set(),order=[];while(order.length<problem.tasks.length){const available=problem.tasks.filter(t=>!done.has(t.id)&&t.predecessors.every(p=>done.has(p)));if(!available.length)throw new Error('Cannot create a topological task order.');for(const task of available){done.add(task.id);order.push(task.id);}}return order;}
function topologyRank(problem){return new Map(topologicalOrder(problem).map((id,index)=>[id,index]));}
...
function candidateFromStations(problem,stations,sequence){return evaluateSolution(problem,normalizeStations(problem,stations),sequence);}
function canonicalStations(problem,stations,p,preferredBack=null){
  const assignments=stations.map(station=>[...station.forward,...station.backward]),ids=problem.tasks.map(task=>task.id),flat=assignments.flat();
  ...
  const successors=new Map(ids.map(id=>[id,[]]));for(const task of problem.tasks)for(const predecessor of task.predecessors)successors.get(predecessor).push(task.id);
  const order=topologicalOrder(problem),frontOK=new Map(),backOK=new Map(),canBack=new Map(),preference=preferredBack||new Set(stations.flatMap(station=>station.backward));
  for(const id of order){const v=stationOf.get(id),task=problem.tasks.find(item=>item.id===id);frontOK.set(id,task.predecessors.every(before=>stationOf.get(before)<=v));backOK.set(id,successors.get(id).every(after=>stationOf.get(after)<=v));}
  ...
  for(const id of order){const v=stationOf.get(id),task=problem.tasks.find(item=>item.id===id),forcedBack=p.physicalRoute!==false&&task.predecessors.some(before=>stationOf.get(before)===v&&directions.get(before)==='backward'),front=frontOK.get(id),back=canBack.get(id);let direction=null;if(forcedBack)direction=back?'backward':null;else if(front&&back)direction=p.freeLanes!==false&&preference.has(id)?'backward':'forward';else if(front)direction='forward';else if(back)direction='backward';if(!direction)return null;directions.set(id,direction);}
  return normalizeStations(problem,assignments.map(tasks=>({forward:tasks.filter(id=>directions.get(id)==='forward'),backward:tasks.filter(id=>directions.get(id)==='backward')})));
}
function canonicalCandidate(problem,stations,sequence,p,preferredBack=null){const normalized=canonicalStations(problem,stations,p,preferredBack);return normalized?evaluateSolution(problem,normalized,sequence):null;}
...
function laneFlipNeighbor(problem,current,random,p){
  const base=preferredBack(current),choices=[];for(const task of problem.tasks){const toggled=new Set(base);if(toggled.has(task.id))toggled.delete(task.id);else toggled.add(task.id);const candidate=canonicalCandidate(problem,current.stations,current.sequence,p,toggled);if(candidate&&solutionKey(candidate)!==solutionKey(current))choices.push(candidate);}return choices.length?choices[Math.floor(random()*choices.length)]:current;
}
...
async function runShared(problem,p,progress,stopped,algorithm){
  const random=makeRng(p.seed),pool=collector(algorithm,p.runNumber,p.poolLimitPerRun),history=[],cap=Math.max(1,Math.floor(Number(algorithm==='SA'?p.saMaxIterations:algorithm==='VNS'?p.iterations:p.gaGenerations))),state={iteration:0},sequence=shuffle(minimumPartSet(problem.models),random);
  ...
  while(feasible.stationCount>1&&state.iteration<cap&&!stopped()){
    const merged=canonicalCandidate(problem,mergeMinimumPair(problem,feasible.stations),feasible.sequence,p,preferredBack(feasible));if(!merged)break;consider(merged);if(merged.feasible){feasible=merged;k1=0;k2=0;emit(feasible,algorithm==='SA'?Number(p.saT0):0);continue;}
  ...
  if(!history.length)emit(best,algorithm==='SA'?Number(p.saT0):0);pool.add(best,state.iteration);return{algorithm,best,solutions:pool.values(),history,iterations:state.iteration,poolDropped:pool.dropped()};
}
