...
function evaluate(problem,stations,sequence){
  ...
  const cycleLoads=stations.map(()=>Array(L).fill(0)),cycleSideLoads=stations.map(()=>Array(L));
  for(let j=0;j<K;j++) for(let r=0;r<L;r++){
    ...
    const forwardLoad=stations[j].forward.reduce((s,id)=>s+task.get(id).times[fm],0),backwardLoad=stations[j].backward.reduce((s,id)=>s+task.get(id).times[bm],0),total=forwardLoad+backwardLoad;
    const crossover=forwardLoad>1e-9&&backwardLoad>1e-9,quality=crossover?2*Math.min(forwardLoad,backwardLoad)/total:0;
    cycleLoads[j][r]=total;
    cycleSideLoads[j][r]={station:j+1,forwardModel:fm,backwardModel:bm,forwardLoad,backwardLoad,total,crossover,quality,fbDeviation:Math.abs(forwardLoad-backwardLoad),normalizedDeviation:total?Math.abs(forwardLoad-backwardLoad)/total:0};
  }
  ...
  const stationMetrics=stations.map((station,index)=>{
    const forwardLoad=stationLoad(problem,station.forward),backwardLoad=stationLoad(problem,station.backward);
    const total=forwardLoad+backwardLoad,crossover=forwardLoad>1e-9&&backwardLoad>1e-9;
    const quality=crossover?2*Math.min(forwardLoad,backwardLoad)/total:0;
    return {station:index+1,forwardLoad,backwardLoad,total,crossover,quality,fbDeviation:Math.abs(forwardLoad-backwardLoad),normalizedDeviation:total?Math.abs(forwardLoad-backwardLoad)/total:0};
  });
  const crossoverStations=stationMetrics.filter(x=>x.crossover),crossoverQuantity=crossoverStations.length;
  const crossoverQualityTotal=crossoverStations.reduce((sum,x)=>sum+x.quality,0);
  const crossoverQualityAvg=crossoverQuantity?crossoverQualityTotal/crossoverQuantity:0;
  const crossoverFbad=crossoverStations.reduce((sum,x)=>sum+x.fbDeviation,0);
  const precedenceFeasible=assignmentFeasible(problem,stations);
  return {stations,sequence,cycleLoads,cycleSideLoads,stationMetrics,stationCount:K,cmin,overload,adw,maxLoad,crossoverQuantity,crossoverQualityTotal,crossoverQualityAvg,crossoverFbad,precedenceFeasible,feasible:overload<1e-9&&precedenceFeasible,efficiency:weightedTotal/(K*problem.cycleTime)};
}
...
export function evaluateSolution(problem,stations,sequence){return evaluate(problem,normalizeStations(problem,stations),sequence);}
...
function mutateExplicit(problem,current,p,random){
  ...
  for(let attempt=0;attempt<80;attempt++){
    ...
    if(moveKind<Number(p.sideFlipProbability)){
      const candidates=items.filter(x=>stations[x.j][x.side].length>1||stations[x.j][x.side==='forward'?'backward':'forward'].length>0);
      if(!candidates.length)continue;
      const a=candidates[Math.floor(random()*candidates.length)],other=a.side==='forward'?'backward':'forward';
      stations[a.j][a.side]=stations[a.j][a.side].filter(id=>id!==a.id);stations[a.j][other].push(a.id);
    }else if(random()<Number(p.p2)&&items.length>1){
...
function researchValue(x,p){return Number(p.quantityWeight)*x.crossoverQuantity+Number(p.qualityWeight)*x.crossoverQualityTotal-Number(p.adwWeight)*x.adw;}
function stageEnergy(x,p){return x.overload*1e5+(x.precedenceFeasible?0:1e9)-researchValue(x,p);}
function betterFeasible(a,b,p){return a?.feasible&&(!b?.feasible||a.stationCount<b.stationCount||(a.stationCount===b.stationCount&&(researchValue(a,p)>researchValue(b,p)+1e-9||(Math.abs(researchValue(a,p)-researchValue(b,p))<1e-9&&a.adw<b.adw-1e-9))));}
...
export async function anneal(problem,params,progress,stopped=()=>false){
  ...
  const p={T0:1000,Tmin:1,IT:20,R:.95,p1:.8,p2:.5,p3:.5,sideFlipProbability:.35,quantityWeight:100,qualityWeight:50,adwWeight:.05,maxIterations:300000,seed:42,...params};
  ...
  const emit=(current,T)=>{const point={iteration:state.iteration,T,current:stageEnergy(current,p),best:-researchValue(best,p),overload:best.overload,stations:best.stationCount,quantity:best.crossoverQuantity,quality:best.crossoverQualityAvg,qualityTotal:best.crossoverQualityTotal};history.push(point);progress({point,best,iteration:state.iteration,temperature:T});};
  async function searchStage(initial,stopOnFirstFeasible){
    let current=initial,currentEnergy=stageEnergy(current,p),bestStage=current,bestStageEnergy=currentEnergy,bestStageFeasible=current.feasible?current:null,T=Number(p.T0);
    while(T>=Number(p.Tmin)&&state.iteration<cap&&!stopped()){
      for(let inner=0;inner<Number(p.IT)&&state.iteration<cap&&!stopped();inner++,state.iteration++){
        const candidate=mutateExplicit(problem,current,p,random),candidateEnergy=stageEnergy(candidate,p),delta=candidateEnergy-currentEnergy;
        ...
        if(candidate.feasible&&betterFeasible(candidate,bestStageFeasible,p)){bestStageFeasible=candidate;if(betterFeasible(candidate,best,p))best=candidate;if(stopOnFirstFeasible){emit(candidate,T);return{bestStage,bestFeasible:bestStageFeasible};}}
      }
  ...
  if(!benchmark){
    while(feasible.stationCount>1&&state.iteration<cap&&!stopped()){
      ...
      if(merged.feasible){feasible=merged;if(betterFeasible(feasible,best,p))best=feasible;continue;}
      const reduced=await searchStage(merged,true);
      if(reduced.bestFeasible){feasible=reduced.bestFeasible;if(betterFeasible(feasible,best,p))best=feasible;continue;}
      const smooth=await searchStage(feasible,false);if(betterFeasible(smooth.bestFeasible,feasible,p))feasible=smooth.bestFeasible;if(betterFeasible(feasible,best,p))best=feasible;
      ...
      if(retried.bestFeasible){feasible=retried.bestFeasible;if(betterFeasible(feasible,best,p))best=feasible;continue;}break;
    }
  }else{
    const smooth=await searchStage(feasible,false);if(betterFeasible(smooth.bestFeasible,best,p))best=smooth.bestFeasible;
  }
  if(!history.length)emit(best,Number(p.T0));
  return {best,bestEnergy:-researchValue(best,p),objectiveValue:researchValue(best,p),history,iterations:state.iteration,objective:{quantityWeight:Number(p.quantityWeight),qualityWeight:Number(p.qualityWeight),adwWeight:Number(p.adwWeight)}};
}
